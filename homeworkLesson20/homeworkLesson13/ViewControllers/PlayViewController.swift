...
class PlayViewController: UIViewController {
    ...
    func findMoving(for checker:UIView) {
        ...
        chessboard.subviews.forEach { cellForMove in
            ...
            if currentChecker == .white {
                if cellForMove.tag == startCell.tag + 7 || cellForMove.tag == startCell.tag + 9 {
    ...
    func findFight(for checker:UIView) {
        let cell = checker.superview
        guard let startCell = cell else { return }
        chessboard.subviews.forEach { cellWithWhiteChecker in
            if currentChecker == .white {
                if cellWithWhiteChecker.tag == startCell.tag + 7 {
                    cellWithWhiteChecker.subviews.forEach { checker in
                        guard checker.tag == 1 else {return}
                    }
                }
            }
